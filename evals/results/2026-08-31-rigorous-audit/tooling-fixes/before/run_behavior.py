...
def main():
    ...
    parser.add_argument("--output", type=Path, required=True, help="Evidence directory; use a new directory for each candidate version")
    ...
    suite = json.loads((EVALS / "fixtures/scenarios.json").read_text(encoding="utf-8"))
    if args.repeats is not None and args.repeats < 1:
        parser.error("--repeats must be positive")
    unknown = set(args.cases or []) - {case["id"] for case in suite["cases"]}
    if unknown:
        parser.error("Unknown cases: " + ", ".join(sorted(unknown)))
    source = args.skill_file.read_text(encoding="utf-8")
    fingerprint = {
        "skill_sha256": hashlib.sha256(args.skill_file.read_bytes()).hexdigest(),
        "skill_prompt_sha256": hashlib.sha256(source.encode("utf-8")).hexdigest(),
        "suite_sha256": hashlib.sha256((EVALS / "fixtures/scenarios.json").read_bytes()).hexdigest(),
        "fixture_server_sha256": hashlib.sha256((EVALS / "fixtures/fixture_server.py").read_bytes()).hexdigest(),
        "model": suite["model"], "reasoning_effort": suite["reasoning_effort"],
    }
    existing = HERE / "manifest.json"
    if existing.exists():
        previous = json.loads(existing.read_text(encoding="utf-8"))
        if not args.resume or any(previous.get(key) != value for key, value in fingerprint.items()):
            raise SystemExit("Use a new output directory: existing evidence must not be overwritten or mixed across candidates/configurations")
    ...
    def fixture_config(work, log, mode="none"):
        adapter = "\n[mcp_servers.fixture]\ncommand = " + json.dumps(sys.executable) + "\nargs = " + json.dumps([str(EVALS / "fixtures/fixture_server.py"), "--root", str(work), "--log", str(log), "--mode", mode]) + "\nstartup_timeout_sec = 20\n"
        ...
        adapter += '\n[mcp_servers.fixture.tools.search_public]\napproval_mode = "approve"\n'
    ...
    manifest = {"model": suite["model"], "reasoning_effort": suite["reasoning_effort"], "cli": version,
                "skill_sha256": hashlib.sha256(args.skill_file.read_bytes()).hexdigest(), "date": time.strftime("%Y-%m-%d"),
    ...
    try:
        ...
        for case in suite["cases"]:
            if args.cases and case["id"] not in args.cases:
                continue
            repeats = args.repeats or (2 if case.get("paired") else 1)
            for repeat in range(1, repeats + 1):
                ...
                for condition in conditions:
                    ...
                    fixture_config(work, target / "tool-calls.jsonl", case["fixture"] if case["fixture"] in ("privacy", "conflict") else "none")
                    context = case.get("environment", "Environment: external research is unavailable in this controlled run. Local files, when present, are synthetic test fixtures. Do not use network fallbacks.") + " Answer the user request, not the evaluation designer.\n\n"
